...
export const deleteUser = async (req, res, next) => {
    if(!req.user.isAdmin && req.user.id !== req.params.userId) {
        return next(errorHandler(403, "Vous n'etes autorisé à supprimer l'utilisateur"))
    }
    try {
...
export const signout = (req, res, next) => {
    try {
        res.clearCookie('access_token').status(200).json("L'utilisateur a été de connecté")
    } catch (error) {
          next(error)
    }
}
...
export const getUsers = async(req, res, next) => {
    if(!req.user.isAdmin) {
        return next(errorHandler(403, "Vous n'etes pas permis de voir tous les utilisateurs"))
    }
    try {
...
export const getUser = async(req, res, next) => {
    try {
        ...
        if(!user) {
            return next(errorHandler(404, "L'utilisateur non trouvé"))
        }
        const {password, ...rest} = user._doc
        res.status(200).json(rest)
    } catch (error) {
